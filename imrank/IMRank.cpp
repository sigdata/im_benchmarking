...
double imrank::Build(char *initialRank, int l, int max_loop, int set_size)
{
	constructEdges();
	...
	do{
		...
		if(diff_top_k || round == max_loop){
		//if(diff_top_k){
		//if(round == IMRank_LOOP || diff_top_k ){
			end_time = clock();
	...
	if(strcmp(initialRank,"Degree")==0){//score = outDegree
	   ...
	   for(int i=0; i<n; i++){
		   ...
		   for(int j=0; j<degree; j++){
			   ...
			   rankNodes[i].score += Graph::edges[begin+1+j].w1;
		   }
	   }
    	...
    	do
    	{
	  		...
	  		for (int j=0;j<dd[i];j++)
			{
				...
				rankNodes[e.v].score+=dp[i]*e.w2/self[i];
			}
	    	delta=0;
	    	for (i=0;i<n;i++)
	    	{
